...
function renderDriverOrders(){
  ...
  box.querySelectorAll("[data-proof-pin]").forEach(b=>{
    b.onclick=()=>verifyDriverProofPin(b.dataset.proofPin);
  });
  box.querySelectorAll("[data-proof-photo]").forEach(b=>{
    b.onclick=()=>uploadDriverProofPhoto(b.dataset.proofPhoto);
  });
  box.querySelectorAll("[data-proof-view]").forEach(b=>{
    b.onclick=()=>viewDeliveryProofMedia(b.dataset.proofView,b.dataset.proofKind);
  });
  box.querySelectorAll("[data-proof-sign-clear]").forEach(b=>{
    b.onclick=()=>clearDriverProofSignature(b.dataset.proofSignClear);
  });
  box.querySelectorAll("[data-proof-sign-upload]").forEach(b=>{
    b.onclick=()=>uploadDriverProofSignature(b.dataset.proofSignUpload);
  });
...
+Number(o.total||0).toFixed(2)+'</p>'+proofPanel+action+'</div>';
  }).join("");
