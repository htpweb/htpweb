...
function clearPlan(){
  $("planId").value="";$("code").value="";$("name").value="";$("description").value="";$("price").value="0";$("durationMonths").value="1";$("order").value="0";$("active").value="true";renderFeatureBuilder();
}
function editPlan(id){
  const p=state.plans.find(x=>x.id===id);if(!p)return;
  $("planId").value=p.id;$("code").value=p.code;$("name").value=p.name;$("description").value=p.description||"";$("price").value=p.price;$("durationMonths").value=String(p.duration_months||1);
...
function renderPlans(){
  ...
  $("assignmentPlan").innerHTML=state.plans.filter(p=>p.active).map(p=>'<option value="'+p.id+'">'+esc(p.name)+' · '+esc(p.duration_months)+' mes(es) · '+money(p.price)+'</option>').join("");
}
...
function renderAssignmentPreview(){
  ...
  let text="Se contratarán "+plan.duration_months+" mes(es) por "+money(plan.price)+" USD.";if(current)text+=" Plan vigente: "+(current.plan_name||current.plan_code)+", vence "+fmtDate(current.ends_at)+".";if($("effectiveMode").value==="AUTO")text+=" HTPWEB aplicará upgrade ahora y renovación/downgrade en el próximo ciclo.";$("assignmentPreview").textContent=text;
}
...
$("savePlan").onclick=async()=>{try{const id=await rpc("master_save_commercial_plan",{p_plan_id:$("planId").value||null,p_code:$("code").value.trim(),p_name:$("name").value.trim(),p_description:$("description").value.trim(),p_price:Number($("price").value),p_duration_months:Number($("durationMonths").value),p_active:$("active").value==="true",p_display_order:Number($("order").value||0),p_entitlements:collectEntitlements()});msg("Plan comercial guardado.");await loadAll();editPlan(id);}catch(e){msg(e.message||"No se pudo guardar el plan.",true);}};
$("assignmentDelivery").onchange=()=>{renderSubscriptionDetail();renderAssignmentPreview();};
$("assignmentPlan").onchange=renderAssignmentPreview;$("effectiveMode").onchange=renderAssignmentPreview;
$("assignPlan").onclick=async()=>{try{const deliveryId=$("assignmentDelivery").value,planId=$("assignmentPlan").value;if(!deliveryId||!planId)throw new Error("Selecciona DELIVERY y plan.");const result=await rpc("master_assign_commercial_plan",{p_delivery_id:deliveryId,p_plan_id:planId,p_effective_mode:$("effectiveMode").value});msg("Plan aplicado: "+(result?.change_type||"OK")+".");await loadAll();$("assignmentDelivery").value=deliveryId;renderSubscriptionDetail();renderAssignmentPreview();}catch(e){msg(e.message||"No se pudo asignar el plan.",true);}};
