-- HTPWEB Código 124 — patrón de carga y recomendación de escalamiento.
-- Añade lecturas/escrituras de API al snapshot horario para diferenciar
-- optimización, más compute, más egress o una futura read replica.

alter table public.platform_resource_settings
  add column if not exists api_read_requests_24h bigint,
  add column if not exists api_write_requests_24h bigint;
...
begin
  ...
  select
    ...
    count(*) filter (where lower(coalesce(o.metadata->>'mimetype','')) like 'image/%')::bigint,
    count(*) filter (
  ...
  into v_storage_bytes,v_storage_objects,v_image_bytes,v_image_objects,v_large_images,v_largest_image_bytes
  ...
  select
    ...
    count(*) filter (where state='active' and query_start < now()-interval '2 seconds')::integer
  ...
  v_egress_used_bytes:=case when v_settings.egress_used_gb is null then null else round(v_settings.egress_used_gb*1073741824)::bigint end;
  v_cached_egress_used_bytes:=case when v_settings.cached_egress_used_gb is null then null else round(v_settings.cached_egress_used_gb*1073741824)::bigint end;
  v_storage_egress_projection_bytes:=case when v_settings.storage_egress_24h_bytes is null then null else v_settings.storage_egress_24h_bytes*30 end;
  v_read_share:=case
    when coalesce(v_settings.api_read_requests_24h,0)+coalesce(v_settings.api_write_requests_24h,0)>0
    then round(100*v_settings.api_read_requests_24h::numeric/(v_settings.api_read_requests_24h+v_settings.api_write_requests_24h),2)
    else null
  end;
  ...
  return jsonb_build_object(
    ...
    'storage_egress_projected_percent',case
      when v_storage_egress_projection_bytes is not null and coalesce(v_egress_quota_bytes,0)>0
    ...
    'api_read_requests_24h',v_settings.api_read_requests_24h,
    'api_write_requests_24h',v_settings.api_write_requests_24h,
    'api_read_share_percent',v_read_share
  );
end;
$$;
