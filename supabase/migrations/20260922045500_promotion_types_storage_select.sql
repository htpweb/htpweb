-- HTPWEB Código 111 — promociones por opciones/combos y SELECT RLS para upsert de imágenes.

alter table public.local_promotions
  add column if not exists promotion_type text not null default 'COMBO';

alter table public.local_promotions
  drop constraint if exists local_promotions_promotion_type_check;

alter table public.local_promotions
  add constraint local_promotions_promotion_type_check
  check (promotion_type in ('COMBO','OPTIONS'));

-- upsert:true de Supabase Storage requiere SELECT además de INSERT/UPDATE.
drop policy if exists htpweb_media_promotion_select on storage.objects;
create policy htpweb_media_promotion_select
on storage.objects
for select
to authenticated
using (
...
create or replace function public.save_local_promotion_v3(
  p_promotion_id uuid,
  p_local_id uuid,
  p_title text,
  p_body text,
...
declare
  ...
  v_type text:=upper(trim(coalesce(p_promotion_type,'COMBO')));
  v_item jsonb;
  v_product_id uuid;
  v_variant_id uuid;
  v_quantity integer;
...
begin
  ...
  if v_type not in ('COMBO','OPTIONS') then
    raise exception 'HTPWEB: tipo de promoción inválido';
  end if;
  ...
  if v_type='OPTIONS' and p_promotion_price is not null then
    raise exception 'HTPWEB: una promoción por opciones no usa precio total; define el precio en cada opción';
  end if;
  ...
  if jsonb_array_length(p_items)=0 then
    raise exception 'HTPWEB: agrega al menos un producto/opción a la promoción';
  end if;
  ...
  insert into public.local_promotions(
    ...
    days_of_week,display_order,active,promotion_type,promotion_price,
  ...
  values(
    ...
    coalesce(p_active,true),v_type,
    case when v_type='OPTIONS' then null else p_promotion_price end,
    auth.uid(),now(),now()
  )
  ...
  delete from public.local_promotion_items where promotion_id=v_id;
  ...
  loop
    ...
    if v_type='OPTIONS' and v_promo_price is null then
      raise exception 'HTPWEB: cada opción promocional debe tener su precio';
    end if;

    insert into public.local_promotion_items(
      promotion_id,product_id,variant_id,quantity,promo_price,display_order
    )
    values(v_id,v_product_id,v_variant_id,v_quantity,v_promo_price,v_order);
...
revoke all on function public.save_local_promotion_v3(
  uuid,uuid,text,text,text,timestamptz,timestamptz,smallint[],integer,boolean,text,numeric,jsonb
) from public,anon;

grant execute on function public.save_local_promotion_v3(
  uuid,uuid,text,text,text,timestamptz,timestamptz,smallint[],integer,boolean,text,numeric,jsonb
) to authenticated;
...
as $$
  select
    p.id,p.local_id,p.title,p.body,p.image_url,p.starts_at,p.ends_at,
    p.days_of_week,p.display_order,p.promotion_type,p.promotion_price,
    coalesce((
      select jsonb_agg(
        jsonb_build_object(
          'id',i.id,
