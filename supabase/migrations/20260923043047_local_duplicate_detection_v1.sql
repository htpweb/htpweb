-- HTPWEB — reconocimiento robusto de LOCAL duplicados.
-- Normaliza nombre/dirección/teléfono y combina señales geográficas antes de crear/importar.

create or replace function public.htp_normalize_identity_text(p_value text)
returns text
language sql
immutable
set search_path=''
...
create or replace function public.htp_normalize_phone(p_value text)
returns text
language sql
immutable
set search_path=''
...
as $$
declare
  v_name text:=public.htp_normalize_identity_text(p_name);
...
begin
  ...
  with base as (
    select
      l.*,
      public.htp_normalize_identity_text(l.name) as n_name,
  ...
  select
    ...
    (
      r.phone_match
      or r.whatsapp_match
      or (r.name_exact and (r.address_exact or (r.d_m is not null and r.d_m<=500)))
      or (r.address_exact and r.d_m is not null and r.d_m<=250)
      or r.match_score>=110
    ) as strong_match,
    array_remove(array[
...
revoke all on function public.master_find_local_duplicates_v1(
  text,text,text,text,numeric,numeric,uuid
) from public,anon;

grant execute on function public.master_find_local_duplicates_v1(
  text,text,text,text,numeric,numeric,uuid
) to authenticated;

comment on function public.master_find_local_duplicates_v1(
  text,text,text,text,numeric,numeric,uuid
) is 'MASTER: detecta LOCAL duplicados por nombre normalizado, teléfono/WhatsApp, dirección y proximidad geográfica.';
